...
proc Stent::Wizard::DrawGeometry {} {
    ...
    set b_points_z(0,0) [expr -1.0 * $wire_diameter]

    for {set j 1} {$j < $num_rows} {incr j} {
        if {[expr $j % 2] eq 0} {
            set tj [expr $j-2]
            set b_points_x(0,$j) $b_points_x(0,$tj)
            set b_points_y(0,$j) [expr $b_points_x(0,$tj) + $point_distance_column]
    ...
    for {set j 0} {$j < $num_rows} {incr j} {
        ...
        for {set i 0} {$i < $num_cols} {incr i} {
            if {$points_x($i,$j) eq 0 && $points_y($i,$j) eq 0 } {
    ...
    GiD_Geometry -v2 create point $cont1 $layer_name $b_points_x(0,0) $b_points_y(0,0) 0.0
    for {set j 0} {$j < $num_rows} {incr j} {
        if {$g eq 1} {set g 0} {set g 1}

        for {set i 0} {$i < $num_cols} {incr i} {
            if {$b_points_x($i,$j) eq 0 && $b_points_y($i,$j) eq 0 } {

            } else {
                incr cont11
                if {$g eq 0 || $j eq $num_rows || $j eq 0} {
    ...
    for {set m [expr 100000 + $number_wires +2]} {$m < [expr $cont11+1]} {incr m} {
        ...
        if {$s eq 1} {
            incr cont3
            set aaa [expr $m -100000]
            set bbb [expr $aaa + $d]
            set ccc [expr $bbb / ($number_wires+1)]
            W $ccc
            W [expr int ($ccc)]
