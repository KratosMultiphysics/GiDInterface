...
class Solution(object):
    ...
    def DefineVariables(self):
        ...
        self.save_intermediate_mechanical_variables = self.ProjectParameters["transfer_results_process"]["save_intermediate_mechanical_variables"].GetBool()
        self.save_intermediate_thermal_variables = self.ProjectParameters["transfer_results_process"]["save_intermediate_thermal_variables"].GetBool()
        self.save_intermediate_variables_step = self.ProjectParameters["transfer_results_process"]["save_intermediate_variables_step"].GetInt()
        self.save_final_mechanical_variables = self.ProjectParameters["transfer_results_process"]["save_final_mechanical_variables"].GetBool()
        self.save_final_thermal_variables = self.ProjectParameters["transfer_results_process"]["save_final_thermal_variables"].GetBool()
        self.add_previous_results = self.ProjectParameters["transfer_results_process"]["add_previous_results"].GetBool()
        self.type_of_results = self.ProjectParameters["transfer_results_process"]["type_of_results"].GetString()
        self.add_displacement = self.ProjectParameters["transfer_results_process"]["add_displacement"].GetBool()
        self.add_stress = self.ProjectParameters["transfer_results_process"]["add_stress"].GetBool()
        self.add_temperature = self.ProjectParameters["transfer_results_process"]["add_temperature"].GetBool()
        self.add_reference_temperature = self.ProjectParameters["transfer_results_process"]["add_reference_temperature"].GetBool()
    ...
    def CreatePostModelPartMechanical(self):
        self.file_name_mechanical = self.ProjectParameters["transfer_results_process"]["file_name_mechanical"].GetString()
    ...
    def CreatePostModelPartThermal(self):
        self.file_name_thermal = self.ProjectParameters["transfer_results_process"]["file_name_thermal"].GetString()
    ...
    def SetSolver(self):
        ...
        full_module_name = python_module_name + "." + self.ProjectParameters["solver_settings"]["solver_type"].GetString()
    ...
    def Initialize(self):
        ...
        # Construct processes to be applied
        import KratosMultiphysics.process_factory
        self.list_of_processes = KratosMultiphysics.process_factory.KratosProcessFactory(self.model).ConstructListOfProcesses( self.ProjectParameters["constraints_process_list"] )
        self.list_of_processes += KratosMultiphysics.process_factory.KratosProcessFactory(self.model).ConstructListOfProcesses( self.ProjectParameters["loads_process_list"] )
        self.list_of_processes += KratosMultiphysics.process_factory.KratosProcessFactory(self.model).ConstructListOfProcesses( self.ProjectParameters["temperature_by_device_list"] )
        self.list_of_processes += KratosMultiphysics.process_factory.KratosProcessFactory(self.model).ConstructListOfProcesses( self.ProjectParameters["output_device_list"] )
        ...
        while( (self.time+self.tol) <= self.end_time ):
            ...
            if self.time == (self.save_intermediate_variables_step*self.time_unit_converter):
                ...
                if self.save_intermediate_mechanical_variables:
                    self.save_utility.SaveMechanicalVariables(self.problem_name, self.ProjectParameters, self.main_model_part, self.save_intermediate_variables_step)
                if self.save_intermediate_thermal_variables:
                    self.save_utility.SaveThermalVariables(self.problem_name, self.ProjectParameters, self.main_model_part, self.save_intermediate_variables_step)
    ...
    def Finalize(self):
        ...
        if self.save_final_mechanical_variables:
            self.save_utility.SaveFinalMechanicalVariables(self.problem_name, self.ProjectParameters, self.main_model_part)

        if self.save_final_thermal_variables:
            self.save_utility.SaveFinalThermalVariables(self.problem_name, self.ProjectParameters, self.main_model_part)
