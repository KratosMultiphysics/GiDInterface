...
proc ::Dam::write::GetConstructionDomainProcessDict { } {
    ...
    set data_basenode_activate_parts [[customlib::GetBaseRoot] selectNodes [spdAux::getRoute "DamActivateParts"]]
    set data_basenode_check_temperature [[customlib::GetBaseRoot] selectNodes [spdAux::getRoute "DamCheckTemperature"]]
    ...
    if {[write::isBooleanTrue $activate]} {
        ...
        set params [list gravity_direction reservoir_bottom_coordinate_in_gravity_direction lift_height h_0 aging construction_input_file_name ambient_input_file_name]
        foreach param $params {
            dict set construction_dict $param [write::getValueByNode [$data_basenode selectNodes "./value\[@n='$param'\]"]]
        }

        # Añado al bloque de diccionario de 'construction_dict' los parámetros de <container> interno: 'activate_parts'

        # Añado guiones bajos a los nombres de los submodelparts activados en el modelo.
        set old_name_soil_part [get_domnode_attribute [$data_basenode_activate_parts selectNodes "./value\[@n='name_soil_part'\]"] v]
        set name_soil_part [string map {" " "_"} $old_name_soil_part]

        set old_name_existing_part [get_domnode_attribute [$data_basenode_activate_parts selectNodes "./value\[@n='name_existing_part'\]"] v]
        set name_existing_part [string map {" " "_"} $old_name_existing_part]

        dict set construction_dict activate_soil_part [write::getValueByNode [$data_basenode_activate_parts selectNodes "./value\[@n='activate_soil_part'\]"]]
        dict set construction_dict name_soil_part $name_soil_part
        dict set construction_dict activate_existing_part [write::getValueByNode [$data_basenode_activate_parts selectNodes "./value\[@n='activate_existing_part'\]"]]
        dict set construction_dict name_existing_part $name_existing_part

        # Añado al bloque de diccionario de 'construction_dict' los parámetros de <container> interno: 'activate_parts' y 'check_temperature'
        ...
        foreach param_check $params_check {
            dict set construction_dict $param_check [write::getValueByNode [$data_basenode_check_temperature selectNodes "./value\[@n='$param_check'\]"]]
        }
        ...
        if {$source_type eq "NonAdiabatic"} {
            ...
            set params [list activation_energy gas_constant constant_rate alpha_initial young_inf q_total A B C D]; # decomposition_threshold]
            foreach param $params {
