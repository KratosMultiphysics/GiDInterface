...
proc ::Dam::xml::CustomTree { args } {
    ...
    if {$nodal_results_base ne ""} {
        set delete_list [list "INITIALTEMPERATURE" "BOFANGTEMPERATURE" "CONSTANTRESERVOIRTEMPERATURE" "MONITORINGRESERVOIRTEMPERATURE"]
...
#         set add_list [list ADDED_MASS VOLUME_ACCELERATION POINT_LOAD FORCE_LOAD FORCE_LOAD POSITIVE_FACE_PRESSURE NODAL_CAUCHY_STRESS_TENSOR NODAL_JOINT_WIDTH Vi_POSITIVE Viii_POSITIVE NODAL_YOUNG_MODULUS]
...
proc ::Dam::xml::ProcCheckConditionState {domNode args} {
    ...
    # Active part for ConsiderSelfweight if it is activated 'Yes'
    if {$resp} {
        if {$cond_id in [list "SelfWeight3D" "SelfWeight2D"]} {
            set ConsiderSelfweight [get_domnode_attribute [$domNode selectNodes [spdAux::getRoute DamConsiderSelfweight]] v]
            if {$ConsiderSelfweight ni [dict get [[Model::getCondition $cond_id] getAttributes ] ConsiderSelfweight]} {set resp 0}
        }
    }

    # Active part for define Grouting model parts if DamActivateConstruction is activated 'Yes'
    if {$resp} {
        if {$cond_id in [list "GroutingTime2D" "GroutingTime3D"]} {
            set Activate_construction [get_domnode_attribute [$domNode selectNodes [spdAux::getRoute DamActivateConstruction]] v]
            if {$Activate_construction ni [dict get [[Model::getCondition $cond_id] getAttributes ] Activate_construction]} {set resp 0}
        }
    }
